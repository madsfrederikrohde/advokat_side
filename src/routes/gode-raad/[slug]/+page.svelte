<script lang="ts">
	...
	const title = $derived(article.seo?.title ?? `${article.title} – ${SITE_NAME}`);
...
<svelte:head>
	...
	<meta property="og:image" content="{SITE_URL}{article.heroImage.src}" />
...
<main data-header-theme="dark" class="bg-white pt-20 sm:pt-24">
	...
	<section data-header-theme="light" class="px-6 pb-20 pt-12 sm:px-8 sm:pt-16 lg:px-12 lg:pb-28">
		<div class="mx-auto grid max-w-[90rem] gap-12 lg:grid-cols-[minmax(0,1.6fr)_minmax(16rem,0.6fr)]">
			<ServiceArticle body={article.body} />
	...
	<section data-header-theme="light" class="px-6 pb-8 sm:px-8 lg:px-12">
		<div class="mx-auto max-w-[90rem]">
			<p class="text-[15px] font-light text-neutral-700">
				Har du spørgsmål om {article.title.toLowerCase()}?
			</p>
			<p class="mt-1 text-[15px] font-light text-neutral-700">
			...
			<div class="mt-6 flex flex-wrap items-center gap-5">
				...
				<div class="flex items-center gap-2 text-sm text-neutral-600">
					...
					<a href="tel:+4520317879" class="font-medium text-[#c43b5e] hover:underline"
						>20 31 78 79</a
					>
				</div>
			</div>
